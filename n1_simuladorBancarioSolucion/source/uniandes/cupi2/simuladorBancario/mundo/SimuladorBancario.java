...
public class SimuladorBancario
{
    ...
    public void consignarCuentaCorriente( double pMonto )
    {
        ...
        consecutivo++;
        Transaccion nueva = new Transaccion( consecutivo, Transaccion.TipoTransaccion.ENTRADA, Transaccion.TipoCuenta.AHORROS, pMonto);
    ...
    public void consignarCuentaAhorros( double pMonto )
    {
        ...
        InvariantesSimuladorBancario();
        consecutivo++;
        new Transaccion( consecutivo, Transaccion.TipoTransaccion.ENTRADA, Transaccion.TipoCuenta.AHORROS, pMonto);
        transacciones.add(nueva);
    }
    ...
    public void retirarCuentaCorriente( double pMonto )
    {
        ...
        InvariantesSimuladorBancario();
        consecutivo++;
        Transaccion nueva = new Transaccion( consecutivo, Transaccion.TipoTransaccion.SALIDA, Transaccion.TipoCuenta.CORRIENTE, pMonto);
        transacciones.add(nueva);
    ...
    public void cerrarCDT( )
    {
        ...
        consecutivo++;
        Transaccion nuevaCDT = new Transaccion ( consecutivo, Transaccion.TipoTransaccion.SALIDA, Transaccion.TipoCuenta.CDT.valorCierreCDT);
        transacciones.add(nuevaCDT);
        
        corriente.consignarMonto( valorCierreCDT );
        consecutivo++;
        Transaccion nuevaCORR = new Transaccion ( consecutivo, Transaccion.TipoTransaccion.ENTRADA, Transaccion.TipoCuenta.CORRIENTE.valorCierreCDT);
        corriente.consignarMonto( valorCierreCDT );
        consecutivo++;
        transacciones.add(nuevaCORR);
    }
    ...
    public void pasarAhorrosToCorriente()
    {
    	...
    	consecutivo++;
    	Transaccion nuevaAhorro = new transaccion(consecutivo,Transaccion.TipoTransaccion.SALIDA,Transaccion.TipoCuenta.AHORROS, Cantidad);
    	transacciones.add(nuevaAhorro);
    	consecutivo++;
    }
   ...
   private boolean CuentaCorrienteValida()
   {
	   return(corriente.darSaldo()>0 || corriente.darSaldo()==0) && corriente!= null; 
   }
   
   private boolean cuentaAhorrosValida()
   {
	   return(ahorros.darSaldo()>0 || corriente.darSaldo()==0) && ahorros!= null; 
   }
   private boolean CDTMasAhorroValida()
   {
	   return calcularSaldoTotal()-corriente.darSaldo()<=INVERSION_MAXIMO;
   }   
   
   private void InvariantesSimuladorBancario()
   {
	   assert CuentaCorrienteValida(): "La cuenta corriente es inválida MVB";
	   assert mesActualValido(): "El mes actual es inválido";
}
