    ...
    public void invertir( double pMontoInvertido, double pInteresMensual, int pMes ) throws Exception
    {
       if(valorInvertido==0)
       {
    	    ...
    	    mesApertura = pMes;
       		...
       		{
    	   	throw new Exception("ERROR: SOLO SE PUEDE TENER UN CTD ACTIVO A LA VEZ");
       		}
      InvariantesCDT();
    }
...
private boolean valorInvertidoValido()
{
	return valorInvertido>0 || valorInvertido==0;
}  
    	
    	
private boolean interesMensualValido()
{
    return interesMensual>0;
}

private boolean mesAperturaValido()
{
	return mesApertura>=1 || mesApertura==0;
}
 
private void InvariantesCDT()
{
	assert valorInvertidoValido( ): "El valor invertido es inválido";
	assert mesAperturaValido():"El mes de apertura es inválido";
}
}
